...
| 11 | Orchestration | LLM (primary) | **Sarvam-M** | Locked | Sarvam-first flagship; Indic-tuned reasoning | GPT-4o, Claude Sonnet, Llama-3.3-70B |
| 12 | Orchestration | LLM (grader / self-critique) | **Claude Haiku 4.5** (Anthropic) | Locked | Avoids circular eval (don't grade Sarvam with Sarvam); cheap, fast | GPT-4o-mini (rejected — Anthropic available), Sarvam-M held-out |
...
| 18 | Ingest | Structured extraction | **Sarvam-M (GPT-4o fallback) with Pydantic structured output + self-critique pass** | Locked | Reliable JSON-mode extraction; self-critique gives per-field confidence | Pure regex, LangChain extraction, Instructor lib |
