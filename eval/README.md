...
| `chunk_sweep_results.json`, `chunk_diagnostic.json` | Outputs of `tools/chunk_sweep.py` — chunk-size / overlap grid. See [ADR-018](../docs/60-decisions/ADR-018-chunk-size-sweep-deferred.md). |
...
| LLM judge (Groq Llama-3.3-70B) | Reply *semantically* answers the question and cites the expected source. Different family from the NIM Qwen brain → non-circular. | `judge_pass` |
...
Per `audit_results/ENTERPRISE_AUDIT.md` D-003: **≥ 90% on the 96-Q gold-QA set is the P0 gate for enterprise deployment.** Current state lives in `results.md`.
...
- [ADR-014](../docs/60-decisions/ADR-014-groq-llama-grader.md) (superseded — but the Groq-judge-of-different-family principle survives)
