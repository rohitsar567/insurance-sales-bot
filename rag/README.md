...
| `ingest.py` | 4. PARSE → 5. CHUNK → 6. EMBED → 7. INDEX | The big one. `read_pdf_pages` (pdfplumber) → `chunk_pages` (800-tok / 120 overlap, sentence-aware) → BGE embed → `chromadb.PersistentClient.add(...)`. Carries the in-process HNSW bloat tripwire ([ADR-029](../docs/60-decisions/ADR-029-disk-storage-hardening.md)). |
...
- `_hf_dataset_backup/rag/{corpus,extracted,vectors}/` — offline canonical mirror of the companion HF Dataset (`rohitsar567/insurance-bot-data`). See [ADR-020](../docs/60-decisions/ADR-020-code-data-split-hf-dataset.md).
...
- [ADR-004](../docs/60-decisions/ADR-004-hybrid-structured-vector.md) — hybrid structured + vector retrieval rationale
- [ADR-011](../docs/60-decisions/ADR-011-bge-local-embeddings.md) — why local BGE replaced Voyage
- [ADR-018](../docs/60-decisions/ADR-018-chunk-size-sweep-deferred.md) — 800/120 chunk-size baseline
- [ADR-020](../docs/60-decisions/ADR-020-code-data-split-hf-dataset.md) — code-vs-data repo split
- [ADR-029](../docs/60-decisions/ADR-029-disk-storage-hardening.md) — HNSW bloat tripwire (3-layer defence)
