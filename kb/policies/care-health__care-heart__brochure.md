---
policy_id: care-health__care-heart__brochure
insurer_slug: care-health
insurer_name: Care Health Insurance
policy_name: "Care Heart"
uin_code: RHIHLIP19066V011819
source_pdf_path: rag/corpus/care-health/care-heart__brochure.pdf
completeness_pct: 36
curated_at: 2026-05-14
---

# Care Heart

**Insurer:** Care Health Insurance (`care-health`)  
**Policy ID:** `care-health__care-heart__brochure`  
**UIN:** `RHIHLIP19066V011819`  
**Curation completeness:** 36%  
**Primary source PDF:** `rag/corpus/care-health/care-heart__brochure.pdf`  
**Curated at:** 2026-05-14

> _Curation note: Curated by tools/curate_remaining.py — pattern-based extraction from local PDF_

## Identity

### UIN code

**Value:** RHIHLIP19066V011819

**Source quote:**

> UIN: RHIHLIP19066V011819

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Policy type

**Value:** group

**Source quote:**

> classified as group from PDF heuristics

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

## Eligibility

### Minimum entry age

**Value:** _not specified_

**Source quote:**

> _(no verbatim quote on record)_

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Maximum entry age

**Value:** _not specified_

**Source quote:**

> _(no verbatim quote on record)_

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Maximum renewal age

**Value:** 99

**Source quote:**

> Life long Quick quote & buy Online renew

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Sum insured options

**Value:** _not specified_

**Source quote:**

> Sum Insured and different Optional

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

## Waiting periods

### Initial waiting period (days)

**Value:** 30

**Source quote:**

> Initial Waiting
30 Days

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Pre-existing disease waiting (months)

**Value:** _not specified_

**Source quote:**

> _(no verbatim quote on record)_

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Maternity waiting (months)

**Value:** _not specified_

**Source quote:**

> _(no verbatim quote on record)_

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

## Coverage scope

### Pre-hospitalization (days)

**Value:** 30

**Source quote:**

> Pre-Hospitalization for 30 days

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Post-hospitalization (days)

**Value:** 60

**Source quote:**

> Post-Hospitalization for 60 days

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Day-care treatments covered

**Value:** _not specified_

**Source quote:**

> _(no verbatim quote on record)_

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### AYUSH coverage

**Value:** _not specified_

**Source quote:**

> _(no verbatim quote on record)_

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Maternity coverage

**Value:** _not specified_

**Source quote:**

> _(no verbatim quote on record)_

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Restoration benefit

**Value:** _not specified_

**Source quote:**

> _(no verbatim quote on record)_

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Room rent capping

**Value:** _not specified_

**Source quote:**

> _(no verbatim quote on record)_

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

## Cost-share

### Co-payment (%)

**Value:** 30

**Source quote:**

> Co-payment
medical and alternative treatments such as Ayurveda, Unani, Sidha and 30%

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

## Claims & service

### Network hospital count

**Value:** _not specified_

**Source quote:**

> _(no verbatim quote on record)_

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Cashless treatment supported

**Value:** Yes

**Source quote:**

> Cashless or Reimbursement Facility

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Claim settlement ratio

**Value:** _not specified_

**Source quote:**

> _(no verbatim quote on record)_

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

### Cashless TAT (hours)

**Value:** _not specified_

**Source quote:**

> _(no verbatim quote on record)_

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`

## Bonuses & loyalty

### No-claim bonus (%)

**Value:** _not specified_

**Source quote:**

> _(no verbatim quote on record)_

**Source:** `rag/corpus/care-health/care-heart__brochure.pdf`


---

_Mirrored from `data/policy_facts/care-health__care-heart__brochure.json`. Provenance — every field's verbatim quote and source PDF path is preserved exactly as curated. Do not hand-edit; regenerate via `tools/build_kb_mirror.py`._
